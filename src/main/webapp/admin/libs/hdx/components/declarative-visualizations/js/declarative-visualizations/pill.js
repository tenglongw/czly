define(
	[
		...
		return function(element) {
			...
			//tweenDuration = el.data('duration') || 0; // Unused?
			...
			var isEmptyBar = d3.sum(values)===0;
			if(isEmptyBar)
			{
				values[0] = '1';
			}
			...
			for(i = 0; i<values.length; ++i)
			{
				...
				var w = xscale(val), w3 = w - 3;
				var isFirst = i===first ? 5 : 0;
				var isLast = i===last ? 5 : 0;
				g.append('path')
					.attr(
					'd', function() {
						//noinspection JSReferencingMutableVariableFromClosure
						return roundRect(x, y, w3, h, isFirst, isLast, isLast, isFirst);
					})
					.classed('pill', true)
