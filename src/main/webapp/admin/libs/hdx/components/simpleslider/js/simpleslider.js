define(
	['jquery', 'ge-bootstrap'], function($) {
		...
		$.fn.simpleslider = function() {
			return this
				...
				function() {
					...
					if($source.data('simpleslider'))
					{
						return;
					}

					$source.data('simpleslider', true);

					var $parent = $source.parent(),
						$target = $(
								'<div>' +
								'<button class="less slider-button"><i class="icon-ico_minus_sm"> </i></button>' +
								'<div class="center">' +
								'<div class="content">' +
								'<label></label>' +
					...
					value.stepSize = Math.abs(value.range) / (trackWidth / 4);
					...
					var updateThumbPosition = function() {
						...
						thumbPosition.current = ((value.current - value.min ) / value.range) * thumbPosition.max;
					...
					var updateValue = function(newValue) {
						...
						$source.val(newValue);
						$source.trigger('change', [newValue, 100 * newValue / value.range, value.min, value.max]);
					};
						...
						'mousemove touchmove', function(e) {
							...
							if(draginfo.startS)
							{
								...
								var newPos = Math.max(Math.min(draginfo.startL + (touchClientX - draginfo.startX), thumbPosition.maxPX), 0),
									newValue = value.range * newPos / thumbPosition.maxPX + value.min;
		...
		$(
			function() {
				$('input[type=range].slider').simpleslider();
			});
	});
